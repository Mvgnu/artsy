  ...
  return (
    <main style={{ background: "#d8d0c1", color: "#242522" }}>
      <section style={{ margin: "0 auto", maxWidth: "92rem", padding: "clamp(5rem, 12vw, 10rem) 1.25rem" }}>
        ...
        <p style={{ fontSize: "clamp(1.2rem, 2.3vw, 1.7rem)", lineHeight: 1.65, margin: "2rem 0 0", maxWidth: "53rem" }}>
          Daumier macht moderne Gesellschaft als gedrängte Öffentlichkeit sichtbar: in der Karikatur, im Gericht, auf der Straße und im Eisenbahnwagen.
        </p>
      </section>

      <section style={{ background: "#2c3130", color: "#eee8da", padding: "clamp(4rem, 9vw, 8rem) 1.25rem" }}>
        <div style={{ margin: "0 auto", maxWidth: "92rem" }}>
          <ArtworkStrip items={daumierProfileArtworks} title="Gewalt, Menge, Transport und Last" />
        </div>
      </section>
      ...
      <section style={{ margin: "0 auto", maxWidth: "78rem", padding: "clamp(5rem, 10vw, 9rem) 1.25rem" }}>
        <div style={{ display: "grid", gap: "1rem", gridTemplateColumns: "repeat(auto-fit, minmax(17rem, 1fr))" }}>
          {phases.map((phase, index) => (
            <article key={phase.title} style={{ borderTop: "1px solid rgb(36 37 34 / .35)", minHeight: "22rem", padding: "1.5rem 0" }}>
      ...
      <section style={{ background: "#9a7760", color: "#f3ebdf", padding: "clamp(5rem, 10vw, 8rem) 1.25rem" }}>
        <div style={{ margin: "0 auto", maxWidth: "78rem" }}>
          ...
          <p style={{ fontSize: "1.2rem", lineHeight: 1.8, margin: "2rem 0 0", maxWidth: "50rem" }}>
            Daumiers Menschen sitzen, warten, reisen und arbeiten nebeneinander. Gerade diese räumliche Nähe macht sichtbar, wie moderne Infrastruktur unterschiedliche Klassen verbindet, ohne ihre Unterschiede aufzuheben.
          </p>
        </div>
      </section>
      ...
      <nav aria-label="Weiterführende Wege" style={{ margin: "0 auto", maxWidth: "92rem", padding: "clamp(4rem, 9vw, 7rem) 1.25rem" }}>
        <div style={{ display: "grid", gap: "1rem", gridTemplateColumns: "repeat(auto-fit, minmax(16rem, 1fr))" }}>
          {links.map(([eyebrow, href, title]) => (
            <Link key={href} href={href!} style={{ border: "1px solid rgb(36 37 34 / .3)", color: "inherit", minHeight: "13rem", padding: "1.5rem", textDecoration: "none" }}>
