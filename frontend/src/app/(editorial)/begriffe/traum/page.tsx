  ...
  return (
    <article style={{ background: "#171521", color: "#f0eaf5", minHeight: "100vh" }}>
      <header style={{ margin: "0 auto", maxWidth: "88rem", padding: "clamp(8rem, 15vw, 13rem) 1.25rem 7rem" }}>
        <p style={{ color: "#c4a3dc", fontSize: ".74rem", fontWeight: 800, letterSpacing: ".2em", textTransform: "uppercase" }}>
          Psychologischer Begriff · Romantik
        </p>
        <h1 style={{ fontFamily: "var(--font-serif)", fontSize: "clamp(4.5rem, 12vw, 10rem)", fontWeight: 500, letterSpacing: "-.06em", lineHeight: ".82", margin: "1.5rem 0 0" }}>
          Der <span style={{ color: "#c4a3dc", fontStyle: "italic" }}>Traum</span>
        </h1>
        <p style={{ color: "rgb(240 234 245 / .74)", fontSize: "clamp(1.2rem, 2.4vw, 1.65rem)", lineHeight: 1.75, margin: "3rem 0 0 auto", maxWidth: "48rem" }}>
          In romantischer Kunst ist der Traum kein bloßer Ausstieg aus der Wirklichkeit. Er kann Angst verdichten, verdrängte Konflikte sichtbar machen, neue Erkenntnis beanspruchen oder vertraute Natur in eine andere Ordnung überführen.
        </p>
      </header>

      <section style={{ background: "#ece5f1", color: "#211b28" }}>
        <div style={{ margin: "0 auto", maxWidth: "88rem", padding: "clamp(5rem, 10vw, 9rem) 1.25rem" }}>
          <h2 style={{ fontFamily: "var(--font-serif)", fontSize: "clamp(3rem, 7vw, 6rem)", fontWeight: 500, letterSpacing: "-.05em", lineHeight: ".95", margin: "0 0 3rem", maxWidth: "13ch" }}>
          ...
          <div style={{ display: "grid", gap: "1rem", gridTemplateColumns: "repeat(auto-fit, minmax(18rem, 1fr))" }}>
            {distinctions.map((item) => (
              <section key={item.title} style={{ border: "1px solid rgb(33 27 40 / .18)", padding: "2rem" }}>
      ...
      <section style={{ background: "#251d31" }}>
        <div style={{ margin: "0 auto", maxWidth: "88rem", padding: "clamp(6rem, 11vw, 10rem) 1.25rem" }}>
          <h2 style={{ fontFamily: "var(--font-serif)", fontSize: "clamp(3rem, 7vw, 6rem)", fontWeight: 500, letterSpacing: "-.05em", margin: 0 }}>
          ...
          <div style={{ display: "grid", gap: "2rem", gridTemplateColumns: "repeat(auto-fit, minmax(20rem, 1fr))", marginTop: "4rem" }}>
            {functions.map((item) => (
              <section key={item.title} style={{ borderTop: "1px solid rgb(240 234 245 / .24)", paddingTop: "1.5rem" }}>
                <h3 style={{ color: "#d9bdea", fontFamily: "var(--font-serif)", fontSize: "2rem", fontWeight: 500, margin: 0 }}>{item.title}</h3>
                <p style={{ color: "rgb(240 234 245 / .72)", lineHeight: 1.8, margin: "1rem 0 0" }}>{item.text}</p>
              </section>
            ))}
          </div>
        </div>
      ...
      <section style={{ background: "#f1ebf4", color: "#211b28" }}>
        <div style={{ margin: "0 auto", maxWidth: "76rem", padding: "clamp(6rem, 11vw, 10rem) 1.25rem" }}>
          <h2 style={{ fontFamily: "var(--font-serif)", fontSize: "clamp(3rem, 7vw, 5.5rem)", fontWeight: 500, letterSpacing: "-.05em", margin: 0 }}>
            Sechs Fragen an ein Traumbild
          </h2>
          <ol style={{ display: "grid", gap: "1rem", listStyle: "none", margin: "3rem 0 0", padding: 0 }}>
            {questions.map((question, index) => (
              <li key={question} style={{ borderBottom: "1px solid rgb(33 27 40 / .18)", display: "grid", gap: "1rem", gridTemplateColumns: "3rem 1fr", padding: "1rem 0" }}>
                <span style={{ color: "#79548f", fontWeight: 800 }}>{String(index + 1).padStart(2, "0")}</span>
      ...
      <footer style={{ margin: "0 auto", maxWidth: "88rem", padding: "5rem 1.25rem 8rem" }}>
        <p style={{ color: "rgb(240 234 245 / .65)", lineHeight: 1.75, maxWidth: "48rem" }}>
          Weiterführend: <Link href="/begriffe/vision-und-prophezeiung/">Vision und Prophezeiung</Link>, <Link href="/begriffe/dunkle-romantik/">Dunkle Romantik</Link> und <Link href="/motive/nacht/">die Nacht</Link>.
        </p>
      </footer>
    </article>
  );
