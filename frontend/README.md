...
The application is a Next.js App Router project using strict TypeScript,
Tailwind CSS, pnpm, ESLint, Prettier, and Vitest.
...
All checks run for frontend pull requests. The current root page is a
non-editorial foundation shell and the site remains `noindex` until complete,
validated editorial routes are available.

## Route groups

- `src/app/(editorial)/` is reserved for movements, artworks, artists, motifs,
  techniques, and approved comparisons.
- `src/app/(discovery)/` is reserved for search, browsing, and curated
  discovery.
